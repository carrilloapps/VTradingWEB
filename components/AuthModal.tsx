      ...
      <DialogContent sx={{ p: { xs: 3, sm: 4 }, textAlign: 'center' }}>
        <Box sx={{ mb: 4 }}>
          ...
          <Typography variant="h5" fontWeight="800" sx={{ letterSpacing: '-0.03em' }}>
            {authMode === 'login' ? 'Bienvenido' : authMode === 'register' ? 'Crea tu cuenta' : 'Recuperar acceso'}
          </Typography>
          <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>
            {authMode === 'login' ? 'Ingresa para monitorear tus mercados' : authMode === 'register' ? 'Únete a la plataforma de monitoreo más avanzada' : 'Enviaremos un enlace a tu correo'}
          </Typography>
        </Box>
        ...
        <form onSubmit={handleEmailAuth}>
          ...
          {authMode === 'login' && (
            <Box textAlign="right" sx={{ mt: 1 }}>
              <Button 
                ...
                sx={{ textTransform: 'none', fontWeight: 600, color: 'primary.main', fontSize: '0.75rem' }}
              >
          ...
          >
            {isProcessing ? <CircularProgress size={24} color="inherit" /> : (authMode === 'login' ? 'Iniciar Sesión' : authMode === 'register' ? 'Registrarse' : 'Enviar Enlace')}
          </Button>
        </form>

        <Box sx={{ my: 3, display: 'flex', alignItems: 'center', gap: 2 }}>
          <Divider sx={{ flexGrow: 1, opacity: 0.5 }} />
          <Typography variant="caption" color="text.disabled" fontWeight="700">O</Typography>
          <Divider sx={{ flexGrow: 1, opacity: 0.5 }} />
        </Box>
        ...
        <Typography variant="body2" sx={{ mt: 4, color: 'text.secondary', fontSize: '0.85rem' }}>
          ...
          <Button 
            ...
            sx={{ ml: 0.5, textTransform: 'none', fontWeight: 800, color: 'primary.main', minWidth: 'auto', p: 0, fontSize: '0.85rem' }}
          >
