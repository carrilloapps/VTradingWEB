...
const ActivityLogs = () => {
  const [logs] = useState([
    { time: '14:02:11', type: 'INFO', message: 'Connected to BCV Gateway.', color: '#3B82F6' },
    {
      time: '14:02:12',
      type: 'SUCCESS',
      message: 'Fetched USD/VES from BCV...',
      status: 'OK',
      color: '#00FF94',
    },
    {
      time: '14:02:15',
      type: 'INFO',
      message: 'Polling IBVC Index session start.',
      color: '#3B82F6',
    },
    {
      time: '14:02:18',
      type: 'SUCCESS',
      message: 'BTC/VES update broadcasted to 12.4k clients.',
      color: '#00FF94',
    },
    {
      time: '14:02:22',
      type: 'WARN',
      message: 'P2P Latency spike detected (+120ms).',
      color: '#F59E0B',
    },
    {
      time: '14:02:25',
      type: 'INFO',
      message: 'Checksum validation for MVZ.A...',
      status: 'VALID',
      color: '#3B82F6',
    },
    {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Typography
          variant="caption"
          sx={{ ml: 1, color: 'text.secondary', fontWeight: 700, fontSize: '0.6rem' }}
        >
          VTR-MONITOR-CLI
        </Typography>
      </Box>
      ...
      >
        {logs.map((log, i) => (
          <Box key={i} sx={{ mb: 1, fontSize: '0.75rem' }}>
            ...
            <Typography
              component="span"
              sx={{ color: log.color, fontWeight: 700, fontSize: 'inherit' }}
            >
              {log.type}:{' '}
            </Typography>
            ...
            {log.status && (
              <Typography
                component="span"
                sx={{ color: log.color, fontWeight: 900, fontSize: 'inherit' }}
              >
                {log.status}
              </Typography>
            )}
  ...
  const services = [
    {
      ...
      source: 'bcv.org.ve',
      uptime: '99.98%',
      status: marketData?.status?.state || 'OPERATIONAL',
      color: marketData?.status?.state === 'CERRADO' ? '#FF3B3B' : '#00FF94',
    },
    {
      name: 'BVC-MARKET-FEED',
      source: 'bolsadecaracas.com',
      uptime: '99.92%',
      status: marketData?.status?.state || 'OPERATIONAL',
      color: marketData?.status?.state === 'CERRADO' ? '#FF3B3B' : '#00FF94',
    },
    {
      name: 'P2P-BIN-AGGREGATOR',
      source: 'p2p.distributed.io',
      uptime: '98.45%',
      status: 'OPERATIONAL',
      color: '#00FF94',
    },
    {
      name: 'AUTH-V3-GATEWAY',
      source: 'internal-aws-us-east',
      uptime: '100.00%',
      status: 'OPERATIONAL',
      color: '#00FF94',
    },
        ...
        >
          <Fade in timeout={1000}>
            <Box sx={{ mb: 8 }}>
              ...
              <Typography
                variant="body1"
                sx={{ color: 'text.secondary', maxWidth: 600, fontSize: '1.1rem', lineHeight: 1.6 }}
              >
                Real-time status of data ingestion clusters, gateway nodes, and distributed service
                health.
              </Typography>
            </Box>
          </Fade>
          ...
          <Grid container spacing={4} sx={{ mb: 8 }}>
            ...
            <Grid size={{ xs: 12, md: 4 }}>
              ...
              >
                ...
                <Chip
                  label={
                    marketData?.status?.state === 'CERRADO'
                      ? 'Market Closed'
                      : 'All Systems Operational'
                  }
                  sx={{
                    bgcolor: alpha(
                      marketData?.status?.state === 'CERRADO' ? '#FF3B3B' : '#00FF94',
                      0.1
                    ),
                    color: marketData?.status?.state === 'CERRADO' ? '#FF3B3B' : '#00FF94',
                    fontWeight: 900,
            ...
            <Grid size={{ xs: 12, md: 8 }}>
              ...
              >
                ...
                >
                  <Box>
                    ...
                    <Typography variant="caption" sx={{ color: 'text.secondary', fontWeight: 600 }}>
                      Active regional data relays & ingestors
                    </Typography>
                  </Box>
                    ...
                    <Grid key={i} size={{ xs: 3, sm: 1.5 }}>
                      <Box
                        sx={{
                          ...
                          borderRadius: 2,
                          bgcolor: alpha(
                            i === 2 || i === 14 ? '#3B82F6' : i === 10 ? '#F59E0B' : '#00FF94',
                            0.1
                          ),
                          border: '1px solid',
                          borderColor: alpha(
                ...
                >
                  ...
                  >
                    VTR-GATEWAY-X1-CLUSTER:{' '}
                    <Box component="span" sx={{ color: '#00FF94' }}>
                      ONLINE
                    </Box>
                  </Typography>
                  <Typography
                ...
                <Table>
                  <TableHead>
                    <TableRow sx={{ bgcolor: alpha(theme.palette.divider, 0.05) }}>
                      {['Service Name', 'Source', 'Uptime', 'Status'].map((head) => (
                        <TableCell
                          key={head}
                          sx={{ py: 2, borderBottom: `1px solid ${theme.palette.divider}` }}
                        >
                          <Typography
                            variant="caption"
                            sx={{
                  ...
                  <TableBody>
                    {services.map((service) => (
                      <TableRow
                        key={service.name}
                        sx={{ '&:hover': { bgcolor: alpha(theme.palette.divider, 0.02) } }}
                      >
                        <TableCell
                        ...
                        >
                          <Typography
                            variant="caption"
                            sx={{ fontFamily: 'JetBrains Mono', color: 'text.secondary' }}
                          >
                            {service.source}
                          </Typography>
                        </TableCell>
